...
async function handleAdminAPI(request, endpoint) {
  ...
  if (endpoint === "pageSize") {
    const pageSize = parseInt(
      await getConfigValue("page_size", CONFIG.PAGE_SIZE)
    );
...
async function checkKeyValidity(key) {
  try {
    // 1. 使用 /v1/models 接口验证密钥有效性（不受模型繁忙影响）
    ...
    if (!validationResponse.ok) {
      ...
      const errorMessage =
        errorData && errorData.message ? errorData.message : "密钥验证失败";
    ...
    if (!balanceResponse.ok) {
      ...
      const errorMessage =
        errorData && errorData.message ? errorData.message : "余额查询失败";
...
async function updateAllKeyBalances() {
  try {
    ...
    // 串行处理以避免触发风控（硅基流动对并发请求敏感）
    const batchSize = 1; // 每次只处理1个密钥
    let updatedCount = 0;
    let failedCount = 0;
    const results = [];
    const now = new Date().toISOString();
...
const MODELS_CACHE_TTL = 60 * 60 * 1000; // 1小时

// 模型列表API处理（带缓存）
async function handleModelsAPI(request) {
  const now = Date.now();
  ...
  try {
    ...
    if (validKeys.length > 0) {
      ...
      selectedKey = validKeys[randomIndex].key;
    }
...
// 模型列表页面处理
async function handleModelsPage(request) {
  return new Response(modelsHtmlContent, {
    headers: {
...
// 模型列表页面HTML内容
const modelsHtmlContent = `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>模型列表 - 硅基流动</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .model-icon {
      ...
      font-weight: 700;
      flex-shrink: 0;
      background: linear-gradient(135deg, #3498db20, #5dade220);
      color: #3498db;
...
<body>
  <div class="container">
    ...
    <div class="stats-bar">
      <div class="stat-item">
        <span class="stat-value" id="totalModels">-</span>
        <span class="stat-label">模型总数</span>
      </div>
      <div class="stat-item">
        <span class="stat-value" id="freeModels">-</span>
    ...
    <div class="main-content">
      <div class="sidebar">
        <input type="text" class="search-box" id="searchInput" placeholder="搜索模型名称...">
        ...
        <div class="filter-section">
          ...
          <div class="filter-tags" id="typeFilters">
            ...
            <button class="filter-tag" data-filter="chat">对话</button>
            <button class="filter-tag" data-filter="image">图像</button>
            <button class="filter-tag" data-filter="video">视频</button>
            <button class="filter-tag" data-filter="audio">语音</button>
            <button class="filter-tag" data-filter="embedding">向量</button>
            <button class="filter-tag" data-filter="rerank">重排</button>
          </div>
        </div>
        
        <div class="filter-section">
          <div class="filter-title">模型系列</div>
          <div class="filter-tags" id="seriesFilters">
            <button class="filter-tag active" data-series="all">全部</button>
            <button class="filter-tag" data-series="deepseek">DeepSeek</button>
            <button class="filter-tag" data-series="qwen">Qwen</button>
            <button class="filter-tag" data-series="glm">GLM</button>
            <button class="filter-tag" data-series="kimi">Kimi</button>
            <button class="filter-tag" data-series="minimax">MiniMax</button>
            <button class="filter-tag" data-series="yi">Yi</button>
          </div>
        </div>
        ...
        <div class="filter-section">
          ...
          <div class="filter-tags" id="priceFilters">
            <button class="filter-tag active" data-price="all">全部</button>
            <button class="filter-tag" data-price="free">免费</button>
            <button class="filter-tag" data-price="paid">付费</button>
          </div>
        </div>
      </div>
  ...
  <script>
    // 模型数据（从API获取或使用预定义数据）
    let allModels = [];
    let currentFilter = { type: 'all', series: 'all', price: 'all', search: '' };
    
    // 模型类型映射
    const modelTypes = {
      'chat': ['Instruct', 'Chat', 'Thinking', 'R1', 'V3', 'Coder', 'QwQ', 'GLM-4', 'Kimi', 'MiniMax-M', 'Yi-'],
      'image': ['FLUX', 'Stable-Diffusion', 'Image', 'SDXL', 'Kolors'],
      'video': ['Video', 'CogVideo', 'Wan', 'Hunyuan'],
      'audio': ['TTS', 'Speech', 'Whisper', 'Audio', 'cosyvoice', 'FishAudio'],
    ...
    function getModelType(modelId) {
      for (const [type, keywords] of Object.entries(modelTypes)) {
        if (keywords.some(kw => modelId.toLowerCase().includes(kw.toLowerCase()))) {
          return type;
        }
      }
      return 'chat';
    ...
    function getModelSeries(modelId) {
      ...
      if (lower.includes('glm') || lower.includes('thudm') || lower.includes('zai-org')) return 'glm';
    ...
    // 判断是否免费（带 Pro/ 前缀的是付费模型）
    function isFreeModel(modelId) {
      return !modelId.startsWith('Pro/');
    }
    
    // 获取模型图标颜色
    function getModelColor(series) {
      const colors = {
        'deepseek': '#4285f4',
        'qwen': '#ff6b35',
        'glm': '#00a67e',
      ...
      return colors[series] || colors.other;
    }
    ...
    function renderModels(models) {
      ...
      grid.innerHTML = models.map(model => {
        ...
        const color = getModelColor(series);
        const provider = model.id.split('/')[0];
        const typeLabel = type === 'chat' ? '对话' : type === 'image' ? '图像' : type === 'video' ? '视频' : type === 'audio' ? '语音' : type === 'embedding' ? '向量' : '重排';
        
        return '<div class="model-card" data-model="' + model.id + '">' +
          '<div class="model-header">' +
            '<div class="model-icon" style="background: ' + color + '20; color: ' + color + ';">' +
          ...
          '<div class="model-tags">' +
            '<span class="model-tag ' + (free ? 'tag-free' : 'tag-paid') + '">' + (free ? '免费' : '付费') + '</span>' +
            '<span class="model-tag tag-type">' + typeLabel + '</span>' +
          '</div>' +
          '<button class="copy-name-btn" onclick="copyModelName(&quot;' + model.id + '&quot;)">复制模型名称</button>' +
        '</div>';
      }).join('');
      
      document.getElementById('filteredCount').textContent = models.length;
    }
    ...
    function applyFilters() {
      ...
      if (currentFilter.type !== 'all') {
        filtered = filtered.filter(m => getModelType(m.id) === currentFilter.type);
      }
      
      // 系列筛选
      if (currentFilter.series !== 'all') {
        filtered = filtered.filter(m => getModelSeries(m.id) === currentFilter.series);
      }
      
      // 价格筛选
      if (currentFilter.price === 'free') {
        filtered = filtered.filter(m => isFreeModel(m.id));
      } else if (currentFilter.price === 'paid') {
        filtered = filtered.filter(m => !isFreeModel(m.id));
      }
    ...
    function initFilters() {
      // 类型筛选
      document.querySelectorAll('#typeFilters .filter-tag').forEach(btn => {
        btn.addEventListener('click', () => {
          document.querySelectorAll('#typeFilters .filter-tag').forEach(b => b.classList.remove('active'));
      ...
      // 系列筛选
      document.querySelectorAll('#seriesFilters .filter-tag').forEach(btn => {
        btn.addEventListener('click', () => {
          document.querySelectorAll('#seriesFilters .filter-tag').forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          currentFilter.series = btn.dataset.series;
          applyFilters();
        });
      });
      
      // 价格筛选
      document.querySelectorAll('#priceFilters .filter-tag').forEach(btn => {
        btn.addEventListener('click', () => {
          document.querySelectorAll('#priceFilters .filter-tag').forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          currentFilter.price = btn.dataset.price;
          applyFilters();
        });
      });
    ...
    async function loadModels() {
      try {
        ...
        document.getElementById('freeModels').textContent = allModels.filter(m => isFreeModel(m.id)).length;
        
        applyFilters();
      } catch (error) {
        console.error('加载模型失败:', error);
        document.getElementById('modelsGrid').innerHTML = '<div class="loading">加载失败，请刷新重试</div>';
      }
    }
...
<head>
  ...
  <div class="container">
      <div class="header">
        ...
        <a href="https://github.com/YPYT1/siliconflow-manager" target="_blank" class="github-link">
  ...
  <div class="float-buttons-container">
    <!-- 模型列表按钮 -->
    <a href="/models" class="float-api-btn float-models-btn">
...
"
curl -X POST 'https://siliconflow-manager.ypyt147.workers.dev/v1/chat/completions' \\
...
">复制</button>
    ...
    <span class="comment"># 完整Chat接口地址</span>
    <span class="copy-line" onclick="copyText('https://siliconflow-manager.ypyt147.workers.dev/v1/chat/completions')">https://siliconflow-manager.ypyt147.workers.dev/v1/chat/completions</span>
            </div>
          </div>
          ...
          <div class="contact-info">
            管理员邮箱：<a href="mailto:ypyt147@163.com">ypyt147@163.com</a>
          </div>
  ...
  <div id="copyright-text">版权所有 &copy; 2025 LiamWang</div>
  ...
  <script>
    ...
    window.copyText = function(text) {
      navigator.clipboard.writeText(text)
        .then(() => {
          showToast('已复制: ' + text.substring(0, 50) + (text.length > 50 ? '...' : ''));
        })
        .catch(err => {
...
<!DOCTYPE html>
  ...
  <style>
    * {
    ...
    .range-select-container input:focus {
      outline: none;
      border-color: #3498db;
    }

    .range-select-container button {
      padding: 6px 12px;
      font-size: 0.85rem;
    }
...
<body>
          ...
          <div class="key-panel">
            ...
            <div class="selection-controls">
              ...
              <div class="range-select-container">
                <input type="text" id="range-select-input" placeholder="如: 1-10 或 5,8,12" title="输入序号范围，如：1-10 或 5,8,12-15">
                <button id="range-select-btn" class="action-button" title="选择指定序号">选择</button>
              </div>
            </div>
          </div>
  ...
  <script>
    ...
    // 客户端直接检测API Key余额（使用 /v1/models 接口，不受模型繁忙影响）
    async function clientCheckKeyValidity(key) {
      try {
        // 1. 使用 /v1/models 接口验证密钥有效性
        const validationResponse = await fetch("https://api.siliconflow.cn/v1/models", {
          method: "GET",
          headers: {
            "Authorization": \`Bearer \${key}\`
          }
    ...
    window.checkKeyBalance = async function(key) {
      try {
        ...
        // 优先使用客户端模式直接请求硅基API（更可靠）
        try {
          result = await clientCheckSingleKey(key);
        } catch (clientError) {
          console.log('客户端检测失败，尝试服务器模式:', clientError);
          // 客户端模式失败时，回退到服务器模式
          const response = await fetch('/admin/api/update-key-balance', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // 序号范围选择功能
      document.getElementById('range-select-btn').addEventListener('click', function() {
        const input = document.getElementById('range-select-input').value.trim();
        if (!input) {
          showToast('请输入序号范围', true);
          return;
        }
        
        // 解析序号范围，支持格式: 1-10, 5,8,12, 1-5,8,10-15
        ...
        updateSelectionStatus();
        showToast('已选择 ' + selectedCount + ' 个Key（已清除之前的选择）');
        
        // 清空输入框
        document.getElementById('range-select-input').value = '';
      });
      
      // 回车键触发选择
      document.getElementById('range-select-input').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
          document.getElementById('range-select-btn').click();
        }
      });
